...
public class DigitalBookShop {

    private static DigitalBookShop digit = null;
    private List<Book> books = initializeBooks();
    private List<Order> orders = new ArrayList<>();
    private List<Request> requests = new ArrayList<>();
    private DigitalBookShop shop;

    private DigitalBookShop() {
    }

    public static DigitalBookShop getInstance() {
        if (digit == null) {
            digit = new DigitalBookShop();
        }
        return digit;
    }
    ...
    public void closeOrder(Integer id, LocalDate executionDate) {
    ...
    public List<Order> getCompletedOrdersInPeriod(LocalDate begin, LocalDate end) {
        List<Order> completedOrders = new ArrayList<>();
        for (Order order : orders) {
            if (order.getExecutionDate().isAfter(begin) && order.getExecutionDate().isBefore(end)) {
    ...
    public void getTotalAmountInperiod(LocalDate begin, LocalDate end) {
        int totalAmount = 0;
        for (Order order : orders) {
    ...
    public List<Book> getUnsoldForSixMonths() {
    ...
    public void sortedOrderComparator(List<Order> orders) {
